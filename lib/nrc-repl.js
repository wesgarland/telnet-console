...
const debug = Boolean((process.env.NRC_DEBUG || '').indexOf('telnet') !== -1);
...
exports.start = function nrc$$start(options, ...replHelpers)
{
  ...
  if (options.port)
  {
    const server = telnet.createServer((client) => handleNewClient(client, options)).listen(options.port);
...
function handleNewClient(client, options)
{ 
  ...
  try
  {
    ...
    client.repl = require('repl').start(leafMerge(options, { socket: client, eval: evalWrapperFactory(options) }));
...
function handleNewConnection(connection, ci, registry)
{
  ...
  if (!client)
    throw new Error('client registry corrupted');
  ...
  function handleConsoleEvents(ev)
  {
    ...
    {
      /*
      for (let i = 0; i < ev.inspectedArguments.length; i++)
      {
        /* prefer low memory footprint over fast i/o... (help me Nagle Timer, you're my only hope) *
        if (i)
          connection.write(' ');
        connection.write(ev.inspectedArguments[i]);
      }
      connection.write('\r\n');
      */
      //      console[ev.level].apply(null, ev.arguments);
      let argv = Array.from(ev.arguments);
      argv.push('\r');
      console[ev.level].apply(null, argv);
    }
    catch(error)
    {
 ...
 */
function evalWrapperFactory(options)
{
