...
exports.start = function nrc$$start(options, ...replHelpers)
{
  ...
  if (options.port)
  {
      ...
      try {
        ...
        client.repl.on('exit', () => {
          try {
            ...
            global.telnetd.clients.splice(telnetd.clients.indexOf(client), 1);
          }
        });
      } catch(e) {
        console.error(e);
    ...
    const server = telnet.createServer(serverMain).listen(options.port);
...
function handleNewConnection(connection, ci, registry)
{
  ...
  console.log('XXX handling new connection');
  debugger;
  ...
  function cleanup()
  {
    registry.connections.splice(registry.clients.indexOf(connection), 1);
    registry.clients    .splice(registry.clients.indexOf(client),     1);
  ...
  connection.on('any', () => console.log('XXX', arguments));
