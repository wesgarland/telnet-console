...
function handleNewClient(client, ci, options)
{
  ...
  function handleConsoleEvents(ev)
  {
    if (client.logOff)
      return;
    beforeLogHandler(client);
    writeLogEntry(client, ev);
    afterLogHandler(client);
  }
...
function beforeLogHandler(client)
{
  ...
  {
    client.repl.pause();
    client.write('\b'.repeat(prompt.length + client.repl.line.length));
    client.write(' ' .repeat(prompt.length + client.repl.line.length));
    client.write('\b'.repeat(prompt.length + client.repl.line.length));
  }
  catch(error)
  {
...
function afterLogHandler(client)
{
  const prompt = client.repl.getPrompt ? client.repl.getPrompt() : client.repl._prompt;
  ...
  try
  {
    client.write(prompt + client.repl.line);
    client.write('\b'.repeat(Math.max(lineLen - Math.min(cursor, lineLen), 0)));
    client.repl.resume();
  } 
  catch(error)
  {
...
{
  registry.forEach(c => client.write(`${c.remoteAddress}\t${c.startTime.toLocaleTimeString('en-CA', { hour12: false })}\r\n`));
